...
const handleSpecialCard = (card: UnoCard) => {
  switch (card.value) {
    ...
    case 'Wild Draw Four':
      ...
      setCurrentCard({ ...card, color: UNO_COLORS[Math.floor(Math.random() * 4)] });
      nextTurn();
      break;
    case 'Wild':
      // In a real game, the current player would choose the color here
...
return (
  <Box flexDirection="column">
    ...
    {players.slice(1).map((hand, playerIndex) => (
      <Text key={playerIndex}>Player {playerIndex + 2}: {hand.length} cards</Text>
    ))}
    <Text>{message}</Text>
    <Text>Press 'd' to draw a card, or the number of the card you want to play.</Text>
  </Box>
);
```
